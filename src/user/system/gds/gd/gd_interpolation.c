			...
			if ((m > 0) && (m < src_h - 1) && (n > 0) && (n < src_w-1)) {
				...
				for (k=-1; k<3; k++) {
					...
					for (l=-1;  l< 3; l++) {
						...
						if ((src_offset_x[_k] <= 0) || (src_offset_y[_k] <= 0) || ((int)src_offset_y[_k] >= src_h) || ((int)src_offset_x[_k] >= src_w)) {
							c = bgColor;
						} else if ((src_offset_x[_k] <= 1) || (src_offset_y[_k] <= 1) || ((int)src_offset_y[_k] >= (int)src_h - 1) || ((int)src_offset_x[_k] >= (int)src_w - 1)) {
