...
void DrawMenuButton(uint32_t x, uint32_t y, bool active, bool down = false){
	...
	GDS_Text(x + symLength + 5, y + buttonHeight - 5, "Menu", menuButtonFont, 9, GetColour(SymbolColour));
}
...
uint64_t DrawTitleBar(uint32_t w, string title, bool active, WindowArea pressed){
	...
	GDS_Text(GetMetric(MenuButtonWidth) + GetMetric(TitleTextMargin), GetMetric(TitleBarSize) - GetMetric(TitleTextBaseline), title.c_str(), font, GetMetric(TitleFontSize), GetColour(TitleTextColour), 0);
