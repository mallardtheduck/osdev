...
extern "C" size_t getline(char **lineptr, size_t *n, FILE *stream){
	...
	if(*n < s.length() + 1 || !*lineptr) *lineptr = (char*)realloc(*lineptr, s.length() + 1);
	memset(*lineptr, 0, s.length() + 1);
	mempcpy(*lineptr, s.c_str(), s.length());
	*n = s.length() + 1;
	return s.length();
}
