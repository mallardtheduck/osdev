...
void RefreshPackage(const string &infpath, const string &path){
    auto file = btos_api::ReadIniFile(infpath);
    if(file.find("package") != file.end()){
        auto pkgsection = file["package"];
        reg::PackageInfo pkginfo;
        ...
        for(auto &s : file){
            if(starts_with(s.first, "feature")){
                ...
                auto cfeat = reg::GetFeatureByName(featinfo.name);
                if(cfeat.id < 0){
                    reg::InstallFeature(featinfo);
                }else{
                    featinfo.id = cfeat.id;
                    reg::UpdateFeature(featinfo);
                }
            }
        }
    }
